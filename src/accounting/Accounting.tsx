...
type Props = { userId: string; language: Language }
...
export function Accounting({ userId, language }: Props) {
  ...
  if (error) return <div role="alert"><p>{t.error}</p><button onClick={() => setRefresh(n => n + 1)}>{t.retry}</button></div>
  ...
  return <div className="books">
    <div className="books-company">
      ...
      <button disabled={locked} onClick={() => setCreate(true)}>＋ {t.newCompany}</button>
    </div>
...
function RecordForm({ language, owner, company, onCreated, onCancel }: {
  ...
  return <form className="books-card books-form" onSubmit={e => { void save(e) }}>
    ...
    <div className="books-buttons"><button className="books-primary" disabled={busy}>{busy ? t.saving : failed ? t.retry : t.create}</button>
...
function Books({ userId, language, company, onLock }: Props & { company: Company; onLock: (locked: boolean) => void }) {
  ...
  return <>
    ...
    <div className="books-filters"><label>{t.from}<input type="date" value={from} onChange={e => { setFrom(e.target.value); setOffset(0) }} /></label>
      ...
      <button onClick={() => setVersion(n => n + 1)} disabled={loading}>{t.refresh}</button></div>
    {message && <p className="books-success" role="status">{message}</p>}
    {command && <section className="books-card books-confirm" role="region" aria-label={t.review}>
      ...
      <div className="books-buttons"><button className="books-primary" disabled={busy} onClick={() => { void save() }}>{busy ? t.saving : attempted ? t.retry : t.confirm}</button>
        {!attempted && <button onClick={() => { setDraft(command); setKind(command.kind); setCommand(null); setSaveError('') }}>{t.edit}</button>}
        {!attempted && <button onClick={() => { setCommand(null); setKind(null); setReversal(null) }}>{t.cancel}</button>}</div>
    ...
    {data && <>
      ...
      <div className="books-actions">{actions.map((action, i) => <button key={action} disabled={Boolean(command)} onClick={() => { setDraft(null); setKind(action); setReversal(null); setSaveError(''); setMessage('') }}><span aria-hidden="true">{['↗', '↙', '＋', '−'][i]}</span>{t[action]}</button>)}</div>
      ...
      {kind && !command && <form key={kind} className="books-card books-form" onSubmit={review}>
        ...
        {saveError && <p role="alert">{saveError}</p>}
        <div className="books-buttons"><button className="books-primary" disabled={!reversal && !allowedParties.length}>{t.review}</button><button type="button" onClick={() => { setKind(null); setReversal(null) }}>{t.cancel}</button></div>
      </form>}
      <section className="books-card"><div className="books-section-title"><h2>{t.parties}</h2><button disabled={Boolean(command)} onClick={() => setAddParty(true)}>＋ {t.newParty}</button></div>
        ...
        <div className="books-party-list">{data.parties.map(p => <button key={p.id} className={party === p.id ? 'selected' : ''} onClick={() => { setParty(p.id); setOffset(0) }}>
      ...
      <section className="books-card"><div className="books-section-title"><h2>{t.ledger}</h2><span>{data.count} {t.entries}</span></div>
        ...
        {data.entries.map(v => <article className="books-entry" key={v.id}>
          ...
          {v.reversed ? <small>{t.reversed}</small> : v.kind !== 'reversal' && <button disabled={Boolean(command)} onClick={() => { setDraft(null); setKind('reversal'); setReversal(v); setSaveError('') }}>{t.reverse}</button>}
        </article>)}
        <div className="books-buttons"><button disabled={offset === 0} onClick={() => setOffset(n => Math.max(0, n - 50))}>{t.previous}</button><span>{Math.floor(offset / 50) + 1}</span><button disabled={offset + 50 >= data.count} onClick={() => setOffset(n => n + 50)}>{t.next}</button></div>
      </section>
